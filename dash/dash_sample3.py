...
app.layout = html.Div([#dcc.Input(id='my-id',value='initial value',type='text'),
   # dcc.Markdown(Markdown_text, dangerously_allow_html=True),
    html.Div([dcc.Dropdown(id='epsilon',options=[{'label':i,'value':i} for i in epsilon],value=epsilon[2])],style={'width': '49%', 'display': 'inline-block'}),
...
def update_figure(selected_epsilon,selected_step):
    #print(r_data[selected_number])
    x = [r_data[epsilon.index(selected_epsilon)][selected_step]]
    y = [e_data[epsilon.index(selected_epsilon)][selected_step]]
    data = [dict(x=x,y=y,text='nmd',mode='markers',marker={'size':20})]
    #print(data)
    x_1 = np.arange(0,10,0.001)
    y_1 = [energy(i,selected_epsilon) for i in x_1]
    #print(y_1)
...
def update_p_figure(selected_epsilon,selected_step):
    x = r_data[epsilon.index(selected_epsilon)][0:selected_step]
    ...
    y_2 = [energy(i,selected_epsilon) for i in x_2]
